...
pygame.display.set_caption("Futebol Pong")
...
def move_ball():
    ...
    if ball_x < 120:  # se for próximo a posição do jogador, verifica
        if player1_y < ball_y + 23:  # a posição do jogador é menor que a bola + metade
            if player1_y + 146 > ball_y:  # posição em relação a dimensão do jogador
                ball_dir *= -1  # há colisão, a bola muda de direção
